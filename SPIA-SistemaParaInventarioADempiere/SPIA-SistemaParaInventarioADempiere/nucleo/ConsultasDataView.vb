...
Public Class ConsultasDataView
    Public Sub ConsultarEquipo(nombre As String)
        ...
        Try
            Dim buscarQuery As String = "Select nombreequipo, nombreresponsable, nombreayudante, usuario, contrasena from equipo inner join usuario on equipo.id_usuario=usuario.id_usuario where nombreequipo Like '%" & nombre & "%'"
        ...
        Catch ex As Exception
            MsgBox(ex.Message, vbCritical, "FrmBuscarEquipo.ConsultarEquipo()")
        End Try
    End Sub
    ...
    Public Sub BuscarProductos(clave As String)
        Dim objConexion As New ConexionDB
        Try
            Dim buscarQuery As String = "SELECT claveproducto, descripcion, unidad FROM m_producto where claveproducto like'%" & clave & "%' or descripcion like '%" & clave & "%'"
            Dim dtDatos As New DataTable
            Dim mdaDatos As New NpgsqlDataAdapter(buscarQuery, objConexion.ConectarSPI())
            mdaDatos.Fill(dtDatos)
